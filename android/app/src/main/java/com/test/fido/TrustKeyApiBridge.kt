...
// 라이브러리 로드 하는 방법을 변경해서 시도 중... 코틀린 싱글톤 패턴으로
object NativeLibrary {
    init {
        // 라이브러리 로드
        Log.d("FidoModule", "Load Trustkey Library...2")
...
class TrustKeyApiBridge(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext)  {

    lateinit var m_callerContext: Context
    private var jniLoader: FidoLibLoader? = null
    ...
    * m_jsonFormatAssertion 입니다 이 모든 일련의  과정은 라이브러리 내부에서 일어납니다 */
    ...
    fun initSetting () {
        ...
        jniLoader?.send_ContextToModule() //<-- Error Occur
        //jniLoader?.setEnvironmentData()
        //jniLoader?.TKAuthN_GetDeviceHandle()
        //Log.d(TAG, com.test.fido.FidoLibLoader.toString())
        //Log.d(TAG, NativeLibrary.toString())
        //NativeLibrary.TKAuthN_GetDeviceHandle()   //<-- Error Occur
    ...
    fun app_hid_read_timeout(readData: ByteArray, readLength: Int, milliseconds: Int) : Int
    {
        ...
        */
        ...
        // 받은 데이타를 모듈 쪽으로 넘깁니다
        jniLoader?.send_HID_ReceivedDataToModule(readData, readLength)
